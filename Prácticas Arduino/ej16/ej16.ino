...
void loop()
{
  ...
  if(num - ultimoControl >= periodoControl){ //Cada periodo de 5000 ms calculamos el error y controlamos el secador y el led Rojo
	...
	prev_error = error;
    if(PID <= 0){ //Si el PID es negativo no hay que encender nada
      digitalWrite(pinRojo, LOW);
      digitalWrite(pinNPN, LOW);
    } else if(PID > 0 && PID < 5){ //Si el PID está entre 0 y 5 encendemos una parte del periodo el secador
      encenderDesde = num;
      tiempoEncendido = 1000.0 * PID;
  ...
  if((num - encenderDesde> tiempoEncendido) && (tiempoEncendido != 0)){ //Controlamos que el tiempo encendido sea distinto de 0 para asegurarnos que el PID esta entre 0 y 5.
